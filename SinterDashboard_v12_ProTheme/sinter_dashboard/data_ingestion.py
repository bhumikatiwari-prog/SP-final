...
def quality_summary_html(agg_df):
    ...
    for t in SQ_TARGETS:
        ...
        ok  = in_pct >= 80
        col = '#5FB878' if ok else '#D4574A'
        rows.append(f"""
        <div style="flex:1;background:#14181C;border:1px solid #3A3530;
                    border-top:3px solid {col};border-radius:8px;padding:12px;text-align:center">
          <div style="font-size:0.68rem;color:#8A8478;margin-bottom:4px">{t}</div>
          <div style="font-size:1.3rem;font-weight:700;color:{col};
                      font-family:'JetBrains Mono',monospace">{s.mean():.3f}</div>
          <div style="font-size:0.65rem;color:#8A8478;margin-top:4px">
...
def render(existing_sq, existing_pp, sq_fe_existing, art):
    ...
    <style>
    .upload-zone {
        background: linear-gradient(135deg,#1A1E23,#1F242A);
        border: 2px dashed #1e4080;
        border-radius:12px; padding:24px 20px;
        text-align:center; transition:all 0.2s;
    }
    .upload-zone:hover { border-color:#2563eb; }
    .check-ok   { color:#5FB878; font-size:0.78rem; margin:3px 0; }
    .check-warn { color:#D4A23B; font-size:0.78rem; margin:3px 0; }
    .check-fail { color:#D4574A; font-size:0.78rem; margin:3px 0; }
    .stat-pill  {
        display:inline-block; background:#1A1E23;
        border:1px solid #3A3530; border-radius:20px;
        padding:3px 12px; font-size:0.7rem; color:#B8B2A6; margin:3px;
    }
    .section-badge {
        background:#1F242A; border-left:3px solid #2563eb;
        padding:8px 14px; font-size:0.78rem; font-weight:700;
        color:#E8C77E; letter-spacing:0.5px;
        border-radius:0 6px 6px 0; margin:14px 0 10px 0;
    }
    .merge-badge {
        background:#052e16; border:1px solid #5FB878;
    ...
    <div style="background:linear-gradient(135deg,#1A1E23,#0f2347);
                border:1px solid #3A3530;border-radius:12px;padding:20px 24px;margin-bottom:18px">
      <div style="font-size:1.1rem;font-weight:700;color:#E8E6E1;margin-bottom:4px">
        📂 Automated Excel Data Aggregation & Processing
      </div>
      <div style="font-size:0.78rem;color:#8A8478;line-height:1.6">
...
| Process Parameters | `Date` | Machine speed, BTP Temp, WGF Speed, Feed, Moisture, ESP Temp/Press |
    ...
    with tab_sq:
        ...
        st.markdown(f"""
        <div style="font-size:0.74rem;color:#8A8478;margin-bottom:12px">
          Expected date column: <code style="color:#E8C77E">DATE</code> &nbsp;·&nbsp;
          Key columns: <code style="color:#E8C77E">TI, RDI, RI, %FeO, %CaO, %MgO, %SiO₂, %Al₂O₃, Basicity&nbsp;(B2)</code>
        </div>""", unsafe_allow_html=True)
    ...
    with tab_pp:
        ...
        <div style="font-size:0.74rem;color:#8A8478;margin-bottom:12px">
          Expected date column: <code style="color:#E8C77E">Date</code> &nbsp;·&nbsp;
          Key columns: <code style="color:#E8C77E">Machine speed, BTP Temperature, WGF Speed, Feed, Moisture, ESP Inlet Temp., %FeO, Basicity(B2)</code>
        </div>""", unsafe_allow_html=True)
    ...
    with tab_both:
        ...
        if combined_file:
            ...
            if sheets:
                st.markdown(f'<div style="font-size:0.74rem;color:#B8B2A6;margin-bottom:8px">'
    ...
    if has_sq or has_pp:
        ...
        if st.session_state.get('data_updated'):
            ...
            <div style="background:#0f2347;border:1px solid #2563eb;border-radius:8px;
                        padding:12px 16px;font-size:0.78rem;color:#E8C77E">
              ...
              &nbsp;<a href="#" style="color:#C9952C">Click Reset below to restore original data.</a>
            </div>""", unsafe_allow_html=True)
...
def _render_validation(df, date_col, num_cols, label):
    ...
    with st.expander(f'{icon} Validation Report — {label}', expanded=not all_ok):
        for r in results:
            ...
            icon2 = '✅' if r['ok'] else '❌'
            detail_html = (f'<span style="color:#6B6458;font-size:0.7rem"> — {r["detail"]}</span>'
                           if r['detail'] else '')
            st.markdown(f'<div class="{css}">{icon2} {r["msg"]}{detail_html}</div>',
                        unsafe_allow_html=True)

        # Column mapping table
        st.markdown('<div style="font-size:0.72rem;color:#8A8478;margin-top:10px;margin-bottom:4px">'
...
def _render_sq_results(agg, fe, stats, existing_sq):
    ...
    if mapping:
        ...
        st.markdown(
            f'<div style="font-size:0.72rem;color:#B8B2A6;background:#1A1E23;'
            f'border:1px solid #3A3530;border-radius:6px;padding:8px 12px;margin-bottom:8px">'
    ...
    # Daily preview table
    st.markdown('<div style="font-size:0.74rem;color:#8A8478;margin:10px 0 4px">Preview — Daily Aggregated Data (first 10 rows)</div>',
                unsafe_allow_html=True)
    ...
    st.markdown(
        f'<div style="font-size:0.74rem;color:#B8B2A6;background:#1A1E23;'
        f'border:1px solid #3A3530;border-radius:6px;padding:10px 14px;margin-top:8px">'
        f'📅 <b>Overlap with existing data:</b> {overlap} common dates &nbsp;·&nbsp; '
        f'<span style="color:#5FB878">{new_only} new dates</span> to be added</div>',
        unsafe_allow_html=True)
    ...
    if n_sd > 0:
        ...
        st.markdown(
            f'<div style="background:#1c0202;border:1px solid #D4574A;border-radius:6px;'
            f'padding:10px 14px;margin:8px 0;font-size:0.75rem;">'
            f'🔴 <b style="color:#D4574A">{n_sd} Shutdown Day(s) Detected</b>'
            f'<span style="color:#B8B2A6"> — tagged in output file: {sd_dates}'
...
def _render_pp_results(agg, fe, stats, existing_pp):
    ...
    if n_sd > 0:
        ...
        st.markdown(
            ...
            f'padding:10px 14px;margin:8px 0;font-size:0.75rem;">'
            f'🔴 <b style="color:#D4574A">{n_sd} Shutdown Day(s) Detected</b>'
            f'<span style="color:#B8B2A6"> — tagged in output: {sd_dates}</span></div>',
            unsafe_allow_html=True)
