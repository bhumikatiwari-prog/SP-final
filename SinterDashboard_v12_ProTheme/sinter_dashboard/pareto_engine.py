...
TEXT  = '#E8E6E1'
GRID  = '#3A3530'
C_OPT = '#5FB878'   # green  = optimal / pareto front
C_TRD = '#D4A23B'   # yellow = trade-off zone
C_AVG = '#4A90D9'   # blue   = average / current
C_BAD = '#D4574A'   # red    = poor solutions
C_REC = '#C9952C'   # purple = recommended point

BASE = dict(
    paper_bgcolor=BG, plot_bgcolor=PLOT,
    font=dict(family='Inter', color=TEXT, size=11),
    hoverlabel=dict(bgcolor='#1A1E23', bordercolor=GRID, font=dict(size=11)),
...
def render(opt_cache, sq_raw):
    ...
    # ── PAGE HEADER ────────────────────────────────────────────────────────
    st.markdown("""
    <div style="background:linear-gradient(135deg,#1A1E23,#0f2347);
                border:1px solid #3A3530;border-radius:12px;
                padding:18px 24px;margin-bottom:18px">
      <div style="font-size:1.05rem;font-weight:700;color:#E8E6E1;margin-bottom:4px">
        ⚡ Multi-Objective Pareto Optimization Engine
      </div>
      <div style="font-size:0.76rem;color:#8A8478;line-height:1.6">
        400 Optuna TPE trials &nbsp;·&nbsp;
        Objectives: <b style="color:#4A90D9">Maximize TI</b> &nbsp;|&nbsp;
        <b style="color:#D4574A">Minimize RDI</b> &nbsp;|&nbsp;
        <b style="color:#5FB878">Maximize RI</b> &nbsp;·&nbsp;
    ...
    kpi_rows = [
        (k1, 'Best TI',         f"{pareto['TI'].max():.3f}",  '#4A90D9', f'Target ≥78'),
        (k2, 'Lowest RDI',      f"{pareto['RDI'].min():.3f}", '#D4574A', f'Target ≤25'),
        (k3, 'Best RI',         f"{pareto['RI'].max():.3f}",  '#5FB878', f'Target ≥68'),
        (k4, 'Pareto Solutions',f"{pareto['_pareto'].sum()}",  '#C9952C', f'of {len(pareto)} trials'),
        (k5, 'Rec. TI / RDI',   f"{rec['TI']:.2f} / {rec['RDI']:.2f}", '#C9952C', 'Best compromise'),
        (k6, 'Rec. RI',         f"{rec['RI']:.3f}",           '#5FB878', 'Best compromise'),
    ]
    for col, label, val, color, sub in kpi_rows:
        col.markdown(
            ...
            f'<div style="font-size:0.62rem;color:#8A8478;text-transform:uppercase;'
            ...
            f'<div style="font-size:0.62rem;color:#6B6458;margin-top:3px">{sub}</div>'
    ...
    with p_tab:
        ...
        fig.add_hrect(y0=0, y1=25, fillcolor='rgba(34,197,94,0.06)',
                      ...
                      row=1, col=1)
        fig.add_vline(x=78, line=dict(color='#4A90D9', width=1.5, dash='dash'),
                      row=1, col=1)
        fig.add_hline(y=25, line=dict(color='#D4574A', width=1.5, dash='dash'),
                      row=1, col=1)

        # Current operating point
        fig.add_trace(go.Scatter(
            x=[cur['TI']], y=[cur['RDI']],
            mode='markers', marker=dict(color='#C9952C', size=14,
        ...
        fig.add_vline(x=68, line=dict(color='#5FB878', width=1.5, dash='dash'), row=1, col=2)
        fig.add_hline(y=78, line=dict(color='#4A90D9', width=1.5, dash='dash'), row=1, col=2)
        ...
        fig.update_xaxes(gridcolor=GRID, zeroline=False, tickfont=dict(size=10),
                         title_font=dict(size=11, color='#8A8478'))
        ...
        fig.add_annotation(
            ...
            font=dict(color='#5FB878', size=10), showarrow=False,
            xref='x', yref='y', row=1, col=1)

        # Color legend explanation
        for col_idx, (clr, lbl) in enumerate([
            ('#5FB878','— Pareto Front'),
            ('#C9952C','◆ Current Operating Point'),
            ('#C9952C','★ Recommended Point'),
            ('#4A90D9','-- TI Target ≥78'),
            ('#D4574A','-- RDI Target ≤25'),
        ], 1):
            fig.add_annotation(
    ...
    with p3d_tab:
        ...
        fig3d.add_trace(go.Scatter3d(
            ...
            mode='markers',
            marker=dict(size=12, color='#C9952C', symbol='diamond',
        ...
        st.markdown(
            '<div style="font-size:0.72rem;color:#8A8478;text-align:center;margin-top:-8px">'
    ...
    with insight_tab:
        ...
        insight_data = [
            ('Best TI achievable',     f"{pareto['TI'].max():.3f}",  f"Current: {cur['TI']:.2f}", '#4A90D9'),
            ('Lowest RDI achievable',  f"{pareto['RDI'].min():.3f}", f"Current: {cur['RDI']:.2f}",'#D4574A'),
            ('Best RI achievable',     f"{pareto['RI'].max():.3f}",  f"Current: {cur['RI']:.2f}", '#5FB878'),
            ('Recommended TI',         f"{rec['TI']:.3f}",  f"Δ {d_ti:+.3f} ({d_ti/cur['TI']*100:+.1f}%)", '#4A90D9'),
            ('Recommended RDI',        f"{rec['RDI']:.3f}", f"Δ {d_rdi:+.3f} ({d_rdi/cur['RDI']*100:+.1f}%)", '#D4574A'),
            ('Recommended RI',         f"{rec['RI']:.3f}",  f"Δ {d_ri:+.3f} ({d_ri/cur['RI']*100:+.1f}%)", '#5FB878'),
        ]
        ...
        for i,(label,val,sub,color) in enumerate(insight_data):
            ...
            brd  = '#5FB878' if good else '#D4574A'
            col.markdown(
                f'<div style="background:{bg};border:1px solid {brd};'
                f'border-radius:8px;padding:12px 16px;margin-bottom:8px">'
                f'<div style="font-size:0.66rem;color:#8A8478;text-transform:uppercase;'
                f'letter-spacing:0.5px;margin-bottom:4px">{label}</div>'
                f'<div style="font-size:1.3rem;font-weight:700;color:{color};'
                f'font-family:JetBrains Mono,monospace">{val}</div>'
                f'<div style="font-size:0.68rem;color:#B8B2A6;margin-top:3px">{sub}</div>'
                f'</div>', unsafe_allow_html=True)

        # Recommended parameter settings
        st.markdown('<div style="background:#1A1E23;border-left:3px solid #C9952C;'
        ...
        if param_rows:
            ...
            st.dataframe(
                rec_df.style.map(
                    lambda v: 'color:#5FB878;font-weight:bold' if isinstance(v,str) and '▲' in v
                              else ('color:#D4574A;font-weight:bold' if isinstance(v,str) and '▼' in v else ''),
                    subset=['Direction']
                ).set_properties(**{
                    'background-color':'#1F242A','color':'#E8E6E1','font-size':'0.8rem'
                }),
        ...
        for col, (t, pct) in zip(i_cols, improvements.items()):
            tc   = {'TI':'#4A90D9','RDI':'#D4574A','RI':'#5FB878'}[t]
            good2= pct > 0
            col.markdown(
                f'<div style="background:#1F242A;border:1px solid #3A3530;'
                f'border-radius:8px;padding:12px;text-align:center">'
                f'<div style="font-size:0.68rem;color:#8A8478;margin-bottom:4px">'
                f'{t} Improvement</div>'
                f'<div style="font-size:1.5rem;font-weight:800;'
                f'color:{"#5FB878" if good2 else "#D4574A"};'
