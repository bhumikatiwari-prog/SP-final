...
.stApp { background: #14181C; color: #E8E6E1; }

/* ── Sidebar — charcoal with subtle gold edge ──────────────────────── */
section[data-testid="stSidebar"] {
    background: linear-gradient(180deg, #1A1E23 0%, #1F242A 100%);
    border-right: 1px solid #3A3530;
}
section[data-testid="stSidebar"] .stSelectbox label,
section[data-testid="stSidebar"] .stSlider label,
section[data-testid="stSidebar"] label { color: #B8B2A6 !important; font-size:0.82rem; }

/* ── Top nav — charcoal with gold accent line ──────────────────────── */
.nav-bar {
    background: linear-gradient(90deg, #1A1E23 0%, #20252B 50%, #1A1E23 100%);
...
.nav-divider {
    width: 1px; height: 38px; background: #3A3530;
}
.nav-title { font-size:1.3rem; font-weight:700; color:#E8E6E1; letter-spacing:0.3px; }
.nav-sub   { font-size:0.72rem; color:#C9952C; letter-spacing:1.2px; text-transform:uppercase; font-weight:600; }
.nav-badge {
    background:#20252B; border:1px solid #C9952C; border-radius:6px;
    padding:5px 14px; font-size:0.72rem; color:#E8C77E; font-weight:500;
}

/* ── KPI Cards — uniform height + aligned grid ─────────────────────── */
.kpi-card {
    background: linear-gradient(135deg, #1F242A 0%, #242A31 100%);
...
.kpi-card:hover { border-color:#C9952C; box-shadow:0 0 18px rgba(201,149,44,0.15); }
.kpi-card::before {
    content:''; position:absolute; top:0; left:0; right:0; height:3px;
    background: var(--accent,#C9952C);
}
.kpi-label { font-size:0.68rem; color:#8A8478; text-transform:uppercase; letter-spacing:1px; margin-bottom:4px; }
.kpi-value { font-size:1.9rem; font-weight:700; color:#E8E6E1; font-family:'JetBrains Mono',monospace; line-height:1.1; }
.kpi-sub   { font-size:0.73rem; margin-top:4px; }
.kpi-ok    { color:#5FB878; }
.kpi-warn  { color:#D4A23B; }
.kpi-bad   { color:#D4574A; }
.kpi-delta { font-size:0.78rem; font-weight:600; margin-top:6px; }

/* ── Section headers — gold left border ────────────────────────────── */
.section-header {
    background: linear-gradient(90deg, #1F242A, transparent);
    border-left: 3px solid #C9952C;
    padding: 9px 16px;
    margin: 18px 0 12px 0;
    font-size: 0.82rem; font-weight:600; color:#B8B2A6;
    text-transform: uppercase; letter-spacing: 1px;
}
.insight-box {
    background: #1F242A; border:1px solid #3A3530;
    border-radius:8px; padding:12px 16px; margin:6px 0;
}
.insight-box h4 { color:#C9952C; font-size:0.82rem; margin:0 0 4px 0; }
.insight-box p  { color:#B8B2A6; font-size:0.8rem; margin:0; line-height:1.5; }

/* ── Tables ─────────────────────────────────────────────────────────── */
.stDataFrame { background:#1F242A !important; }

/* ── Slider / input labels ─────────────────────────────────────────── */
div[data-testid="stNumberInput"] label,
div[data-testid="column"] label { color:#B8B2A6 !important; }

/* ── Plotly chart backgrounds ───────────────────────────────────────── */
.js-plotly-plot .plotly .bg { fill: #1F242A !important; }

/* ── Metric override — equal height + aligned ───────────────────────── */
[data-testid="stHorizontalBlock"] [data-testid="metric-container"] {
    background: #1F242A; border:1px solid #3A3530;
    border-radius:8px; padding:14px 16px;
    height: 100%;
}
[data-testid="stMetricValue"] { color:#E8E6E1 !important; font-family:'JetBrains Mono',monospace; }
[data-testid="stMetricLabel"] { color:#8A8478 !important; }
[data-testid="stMetricDelta"] { font-size:0.8rem !important; }

/* ── Equal-height columns for tile alignment ────────────────────────── */
div[data-testid="column"] { display: flex; flex-direction: column; }
div[data-testid="column"] > div { flex: 1; }

/* ── Tab styling ─────────────────────────────────────────────────────── */
.stTabs [data-baseweb="tab-list"] {
    background: #1A1E23; border-bottom:1px solid #3A3530; gap:4px;
}
...
.stTabs [aria-selected="true"] {
    background: #1F242A !important; color:#E8E6E1 !important;
    border-bottom: 2px solid #C9952C !important;
}
div[data-baseweb="tab-panel"] { background:#14181C; padding-top:16px; }

/* ── Button — amber/gold gradient ───────────────────────────────────── */
.stButton > button {
    background: linear-gradient(135deg, #B8841F, #C9952C);
    color: #14181C; border:none; border-radius:8px;
    padding:10px 24px; font-weight:600; font-size:0.85rem;
    width:100%; transition:all 0.2s;
}
.stButton > button:hover { background: linear-gradient(135deg,#C9952C,#D4A23B); transform:translateY(-1px); }

/* ── Status badges ──────────────────────────────────────────────────── */
.status-ok   { background:#16241A; border:1px solid #5FB878; color:#5FB878; padding:3px 10px; border-radius:20px; font-size:0.72rem; font-weight:600; }
.status-warn { background:#2A2014; border:1px solid #D4A23B; color:#D4A23B; padding:3px 10px; border-radius:20px; font-size:0.72rem; font-weight:600; }
.status-bad  { background:#2A1614; border:1px solid #D4574A; color:#D4574A; padding:3px 10px; border-radius:20px; font-size:0.72rem; font-weight:600; }

hr { border-color:#3A3530; }
</style>
""", unsafe_allow_html=True)

# ── HELPERS ────────────────────────────────────────────────────────────────
CHART_LAYOUT = dict(
    paper_bgcolor='#1F242A', plot_bgcolor='#14181C',
    font=dict(family='Inter',color='#B8B2A6',size=11),
    margin=dict(l=50,r=20,t=40,b=40),
    xaxis=dict(gridcolor='#3A3530',showgrid=True,zeroline=False),
    yaxis=dict(gridcolor='#3A3530',showgrid=True,zeroline=False),
    legend=dict(bgcolor='rgba(0,0,0,0)',bordercolor='#3A3530',font=dict(size=10)),
)
# Professional industrial palette — Blue / Amber-Red / Emerald (colorblind-safe)
TC = {'TI':'#4A90D9','RDI':'#D4574A','RI':'#5FB878'}
TARGETS = ['TI','RDI','RI']
...
def gauge(val, target, direction, title, color):
    ...
    bar_color = color if ok else '#D4574A'
    if direction == '>=':
    ...
    fig = go.Figure(go.Indicator(
        ...
        delta={
            ...
            'valueformat': '.2f',
            'font': {'size': 13},
            'increasing': {'color': '#5FB878'} if direction == '>=' else {'color': '#D4574A'},
            'decreasing': {'color': '#D4574A'} if direction == '>=' else {'color': '#5FB878'},
        },
        number={
            'font': {'size': 20, 'color': '#E8E6E1', 'family': 'JetBrains Mono'},
            'valueformat': '.2f',
            'suffix': '',
        },
        title={
            'text': f'<b>{title}</b>',
            'font': {'size': 11, 'color': '#B8B2A6'},
            'align': 'center',
        },
        ...
        gauge={
            ...
            'bordercolor': '#3A3530',
            'steps': [
                {'range': [lo, ref], 'color': '#20252B'},
                {'range': [ref, hi], 'color': '#1A1E23'},
            ],
            'threshold': {
                'line': {'color': '#C9952C', 'width': 2},
                'thickness': 0.8,
                'value': ref,
            },
        }
    ...
    fig.update_layout(
        paper_bgcolor='#1F242A',
        font=dict(color='#B8B2A6'),
...
# ── Load Jindal Steel logo as base64 for inline embedding ─────────────────
import base64 as _b64
_logo_path = os.path.join(DASHBOARD_DIR, 'assets', 'jindal_logo.png')
_LOGO_B64 = ''
...
# ── SIDEBAR ────────────────────────────────────────────────────────────────
with st.sidebar:
    st.markdown('<div style="text-align:center;padding:16px 0 8px"><span style="font-size:2rem">🏭</span><br><span style="font-size:0.95rem;font-weight:700;color:#E8E6E1">SINTER INTELLIGENCE</span><br><span style="font-size:0.7rem;color:#4a90d9;letter-spacing:1px">PLATFORM v2.0</span></div>', unsafe_allow_html=True)
    if validation_report is not None:
        total = validation_report['n_pass'] + validation_report['n_fail']
        ok    = validation_report['all_ok']
        icon  = '✅' if ok else '⚠️'
        color = '#5FB878' if ok else '#D4A23B'
        st.markdown(
...
<div class="nav-bar">
  <div class="nav-logo-wrap">
    ...
    <div>
      <div class="nav-title">SINTER QUALITY INTELLIGENCE PLATFORM</div>
      <div class="nav-sub">Integrated Steel Plant &middot; Industry 4.0 Analytics</div>
    </div>
  </div>
...
if '🏠' in page:
    ...
    for tab,target in zip([tab1,tab2,tab3],TARGETS):
        with tab:
            ...
            fig.add_hline(y=tgt,line=dict(color='#C9952C',width=1.5,dash='dash'),
    ...
    for col,target,direction in zip([a1,a2,a3],TARGETS,['>=','<=','>=']):
        ...
        <div class="insight-box" style="text-align:center;padding:18px 14px">
          ...
          <div style="font-size:1.9rem;font-weight:700;color:#E8E6E1;
                      font-family:'JetBrains Mono',monospace;line-height:1.1;margin-bottom:4px">
            {recent:.3f}
          </div>
          <div style="font-size:0.71rem;color:#8A8478;margin-bottom:10px">
            Target: {dir_sym}{tgt}
          </div>
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    for target in TARGETS:
        ...
        # Raw
        colors_pts = [TC[target] if ((v>=tgt) if target!='RDI' else (v<=tgt)) else '#D4574A' for v in d2[target]]
        fig.add_trace(go.Scatter(x=d2[date_col],y=d2[target],mode='markers',
        ...
        # Control limits
        for y,name,color,dash in [(ucl,'UCL','#D4A23B','dash'),(lcl,'LCL','#D4A23B','dash'),(mu,'Mean','#8A8478','dot')]:
            fig.add_hline(y=y,line=dict(color=color,width=1,dash=dash),
                          annotation_text=f'{name}:{y:.2f}',row=1,col=1,
                          annotation_font=dict(color=color,size=9))
        fig.add_hline(y=tgt,line=dict(color='#5FB878',width=1.5,dash='dashdot'),
                      annotation_text=f'BF Target: {tgt}',row=1,col=1,
                      annotation_font=dict(color='#5FB878',size=9))
        ...
        fig.update_layout(**clayout(
            ...
            height=420))
        fig.update_yaxes(title_text=target,row=1,col=1,title_font=dict(color='#B8B2A6'))
        fig.update_yaxes(title_text='Freq',row=2,col=1,title_font=dict(color='#B8B2A6'))
        st.plotly_chart(fig, use_container_width=True)
        st.markdown('---')
    ...
    for col,target in zip([ct1,ct2,ct3],TARGETS):
        with col:
            ...
            fig2 = go.Figure(go.Bar(
                ...
                orientation='h',
                marker_color=['#5FB878' if v>0 else '#D4574A' for v in raw_vals.values],
                marker_opacity=0.8,
            ))
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    for col,label,feat,val,tc,hint in [
        (k1,'Top TI Driver',ti_top_feat,ti_top_val,'#4A90D9','Cold strength driver'),
        (k2,'Top RDI Driver',rdi_top_feat,rdi_top_val,'#D4574A','Degradation driver'),
        (k3,'Top RI Driver',ri_top_feat,ri_top_val,'#5FB878','Reducibility driver'),
        (k4,f'Common Drivers',f'{n_common} features',None,'#C9952C',common_labels),
    ]:
        val_html = (f'<div style="font-size:1rem;font-weight:700;color:{tc};'
                    ...
                    f'{feat}</div>'
                    f'<div style="font-size:0.68rem;color:#8A8478">'
                    ...
                    f'<div style="font-size:0.65rem;color:#8A8478">{hint}</div>')
        col.markdown(
            f'<div style="background:#1F242A;border:1px solid #3A3530;'
            f'border-top:3px solid {tc};border-radius:10px;padding:14px 16px;">'
            f'<div style="font-size:0.65rem;color:#8A8478;text-transform:uppercase;'
    ...
    st.markdown(f"""
    <div style="background:linear-gradient(135deg,#1F242A,#1A1E23);
                border:1px solid #1e4080;border-radius:10px;
                padding:16px 20px;margin-bottom:18px">
      <div style="font-size:0.75rem;font-weight:700;color:#C9952C;
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;font-size:0.76rem">
        <div style="background:#14181C;border-radius:6px;padding:10px 12px;border-left:3px solid #4A90D9">
          <span style="color:#4A90D9;font-weight:600">TI:</span>
          <span style="color:#B8B2A6"> {ti_blurb}</span>
        </div>
        <div style="background:#14181C;border-radius:6px;padding:10px 12px;border-left:3px solid #D4574A">
          <span style="color:#D4574A;font-weight:600">RDI:</span>
          <span style="color:#B8B2A6"> {rdi_blurb}</span>
        </div>
        <div style="background:#14181C;border-radius:6px;padding:10px 12px;border-left:3px solid #5FB878">
          <span style="color:#5FB878;font-weight:600">RI:</span>
          <span style="color:#B8B2A6"> {ri_blurb}</span>
        </div>
        <div style="background:#14181C;border-radius:6px;padding:10px 12px;border-left:3px solid #C9952C">
          <span style="color:#C9952C;font-weight:600">Common:</span>
          <span style="color:#B8B2A6"> {common_labels} appear in top-10 for all three targets —
    ...
    with tab_fi:
        st.markdown('<div style="font-size:0.72rem;color:#8A8478;margin-bottom:12px">'
        ...
        for col,target in zip([c1,c2,c3], TARGETS):
            with col:
                ...
                fig = go.Figure(go.Bar(
                    ...
                    textfont=dict(size=9, color='#B8B2A6'),
                    hovertemplate='%{y}<br>Importance: %{x:.4f}<extra></extra>',
                ))
                fig.add_vline(x=0, line=dict(color='#3A3530', width=1))
                fig.update_layout(
                    paper_bgcolor='#1F242A', plot_bgcolor='#14181C',
                    font=dict(family='Inter', color='#B8B2A6', size=10),
                    title=dict(
                    ...
                    height=80 + n_show * 34,
                    xaxis=dict(gridcolor='#3A3530', zeroline=False, tickfont=dict(size=9),
                               title=dict(text='Importance Score', font=dict(size=9,color='#8A8478'))),
                    yaxis=dict(gridcolor='#3A3530', zeroline=False, tickfont=dict(size=9.5),
                               automargin=True),
                )
                st.plotly_chart(fig, use_container_width=True)
                ...
                st.markdown(
                    f'<div style="font-size:0.68rem;color:#8A8478;margin:-6px 0 8px;text-align:center">'
        ...
        for col,target in zip([s1,s2,s3], TARGETS):
            with col:
                ...
                fig.update_layout(
                    ...
                    height=80 + n_show * 34,
                    xaxis=dict(gridcolor='#3A3530', zeroline=False, tickfont=dict(size=9),
                               title=dict(text='Mean |SHAP Value|', font=dict(size=9,color='#8A8478'))),
                    yaxis=dict(gridcolor='#3A3530', zeroline=False, tickfont=dict(size=9.5),
                               automargin=True),
                )
                st.plotly_chart(fig, use_container_width=True)
    ...
    with tab_bee:
        target_sel = st.selectbox('Select Quality Target', TARGETS,
                                   ...
                                   key='bee_target')
        st.markdown(
            '<div style="font-size:0.72rem;color:#8A8478;margin-bottom:10px">'
            'Each dot = one day\'s prediction. '
            '<span style="color:#4A90D9">Blue</span> = low feature value, '
            '<span style="color:#D4574A">Red</span> = high feature value. '
        ...
        fig.add_vline(x=0, line=dict(color='#B8B2A6', width=1, dash='dot'),
                      annotation_text='Baseline', annotation_font=dict(color='#8A8478',size=9))
        fig.update_layout(
            paper_bgcolor='#1F242A', plot_bgcolor='#14181C',
            font=dict(family='Inter', color='#B8B2A6', size=10),
            title=dict(
            ...
            xaxis=dict(gridcolor='#3A3530', zeroline=False, tickfont=dict(size=9),
                       title=dict(text=f'SHAP Value  →  Impact on {target_sel} Prediction',
                                  font=dict(size=10, color='#8A8478'))),
            yaxis=dict(gridcolor='#3A3530', zeroline=False, tickfont=dict(size=9.5),
                       automargin=True),
            hovermode='closest',
        )
    ...
    with tab_dep:
        st.markdown('<div style="font-size:0.72rem;color:#8A8478;margin-bottom:10px">'
            ...
            fig = go.Figure(go.Scatter(
                ...
                marker=dict(color=colors, size=6, opacity=0.6,
                            ...
                            colorbar=dict(title='Feature<br>Value',
                                          tickfont=dict(size=8, color='#B8B2A6'),
                                          titlefont=dict(size=9, color='#B8B2A6'))),
                hovertemplate=f'{dep_sel}: %{{x:.3f}}<br>SHAP Impact: %{{y:.4f}}<extra></extra>',
            ))
            fig.add_hline(y=0, line=dict(color='#B8B2A6', width=1, dash='dot'))
            fig.update_layout(
                paper_bgcolor='#1F242A', plot_bgcolor='#14181C',
                font=dict(family='Inter', color='#B8B2A6', size=10),
                title=dict(
                ...
                xaxis=dict(gridcolor='#3A3530', zeroline=False, tickfont=dict(size=9),
                           title=dict(text=f'{dep_sel}  (Feature Value)',
                                      font=dict(size=10, color='#8A8478'))),
                yaxis=dict(gridcolor='#3A3530', zeroline=False, tickfont=dict(size=9),
    ...
    with tab_wf:
        ...
        if n_rows == 0:
            ...
            fig = go.Figure(go.Waterfall(
                ...
                textposition='outside',
                textfont=dict(size=9, color='#B8B2A6'),
                connector=dict(line=dict(color='#3A3530', width=1)),
                increasing=dict(marker=dict(color='#5FB878')),
                decreasing=dict(marker=dict(color='#D4574A')),
            ...
            fig.update_layout(
                ...
                yaxis=dict(gridcolor='#3A3530', zeroline=False,
                           title=dict(text=wf_target, font=dict(size=10, color='#8A8478'))),
            )
            st.plotly_chart(fig, use_container_width=True)
    ...
    with tab_met:
        for target in TARGETS:
            ...
            data   = sd[target]
            st.markdown(
                f'<div style="background:#1F242A;border-left:4px solid {tc};'
                ...
                f'<span style="color:#8A8478;font-size:0.72rem;margin-left:10px">'
            ...
            for i,(feat,val) in enumerate(zip(data['top_feats'][:n_show],
                ...
                card = (
                    f'<div style="background:#14181C;border:1px solid #3A3530;'
                    ...
                    f'<span style="font-size:0.68rem;background:#1F242A;'
                    f'border:1px solid #3A3530;border-radius:12px;'
                    f'padding:2px 8px;color:#B8B2A6">SHAP: {val:.4f}</span>'
                    f'</div>'
                    f'<div style="font-size:0.73rem;color:#B8B2A6;margin-bottom:4px">'
                    f'🔬 {mech}</div>'
                    f'<div style="font-size:0.71rem;color:#5FB878">'
                    f'✅ {rec}</div>'
                    f'</div>'
                )
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    with col1:
        ...
        st.markdown(f'<div class="insight-box" style="text-align:center"><h4>{status_txt}</h4><p>7d avg: <b>{recent_avg:.3f}</b><br>Historical avg: <b>{hist_avg:.3f}</b><br>Δ = <b style="color:{"#D4574A" if is_issue else "#5FB878"}">{delta:+.3f}</b></p></div>',unsafe_allow_html=True)
    with col2:
        ...
        fig = go.Figure(go.Bar(
            ...
            marker=dict(color=['#D4574A','#D4574A','#D4574A','#D4A23B','#D4A23B','#D4A23B','#5FB878','#5FB878'][::-1],opacity=0.8),
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    st.markdown('<div style="font-size:0.8rem;color:#8A8478;margin-bottom:16px">Adjust process parameters below to predict TI, RDI, and RI instantly.</div>',unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    st.markdown('<div style="color:#B8B2A6;font-size:0.8rem;margin-bottom:12px">Compare a modified scenario against the median/baseline. Use the sliders to modify variables.</div>',unsafe_allow_html=True)
    ...
    fig.add_trace(go.Scatterpolar(r=mod_norm, theta=categories,fill='toself',name='Scenario',
                                   line=dict(color='#D4A23B')))
    fig.update_layout(polar=dict(bgcolor='#1F242A',radialaxis=dict(gridcolor='#3A3530'),
                                  angularaxis=dict(gridcolor='#3A3530')),
                      paper_bgcolor='#1F242A',font=dict(color='#B8B2A6'),
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    st.markdown('<div style="font-size:0.8rem;color:#8A8478;margin-bottom:12px">Runs Optuna NSGA-II multi-objective optimization to find process settings that maximize TI, minimize RDI, and maximize RI simultaneously — producing a genuine Pareto front rather than trials clustered around one fixed trade-off.</div>',unsafe_allow_html=True)
    ...
    if 'opt_result' in st.session_state:
        ...
        st.dataframe(rec_df.style.map(lambda v: 'color:#5FB878' if isinstance(v,str) and '▲' in v
                                       else ('color:#D4574A' if isinstance(v,str) and '▼' in v else ''),
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    b4.plotly_chart(gauge(bf,85,'>=','Overall BF Score','#C9952C'),use_container_width=True)
    ...
    for target, val, tgt, direction, desc, risk, action in bf_impacts:
        ...
        <div style="background:#{color_hex};border:1px solid #{border_hex};border-radius:10px;padding:14px 18px;margin:8px 0">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
            ...
            <span style="color:{'#5FB878' if ok else '#D4574A'};font-size:0.8rem;font-weight:600">Risk: {risk_level}</span>
          </div>
          <p style="color:#B8B2A6;font-size:0.8rem;margin:0 0 6px 0">📌 <b>Description:</b> {desc}</p>
          <p style="color:#D4A23B;font-size:0.8rem;margin:0 0 6px 0">⚠️ <b>BF Risk:</b> {risk}</p>
          <p style="color:#5FB878;font-size:0.8rem;margin:0">✅ <b>Recommended:</b> {action}</p>
        </div>""", unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    with tab4:
        ...
        for category,action,priority,basis in actions:
            p_color = '#D4574A' if priority=='High' else ('#D4A23B' if priority=='Medium' else '#5FB878')
