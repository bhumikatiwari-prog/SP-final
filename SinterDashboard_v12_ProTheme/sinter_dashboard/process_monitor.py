...
CLR = dict(
    ...
    text       = '#E8E6E1',
    muted      = '#8A8478',
    compare    = ['#4A90D9','#C9952C','#5FB878','#C9952C','#06b6d4','#D4A23B'],
)
...
def stat_card_html(label, col, stats, health, badge):
    ...
    delta_col= '#5FB878' if delta > 0 else '#D4574A'
    bdr_col  = {'normal': '#1e4080', 'warning': '#92400e', 'critical': '#7f1d1d'}[health]
    top_col  = {'normal': '#4A90D9', 'warning': '#D4A23B', 'critical': '#D4574A'}[health]
    anom_col = '#D4574A' if anom > 0 else '#5FB878'
    anom_txt = f'{anom} anomaly detected' if anom > 0 else '0 anomalies'

    return f"""
    <div style="background:#1F242A;border:1px solid {bdr_col};
      ...
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:10px">
        <div>
          ...
          <div style="font-size:0.65rem;color:#6B6458;line-height:1.4">{desc}</div>
        </div>
        <div style="font-size:0.85rem;flex-shrink:0;margin-left:8px">{badge}</div>
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-bottom:10px">
        <div style="text-align:center;background:#14181C;border-radius:6px;padding:8px 4px">
          <div style="font-size:0.6rem;color:#8A8478;margin-bottom:3px;letter-spacing:0.5px">CURRENT</div>
          <div style="font-size:1.1rem;font-weight:700;color:#E8E6E1;
                      font-family:'JetBrains Mono',monospace;line-height:1">{cur:.3f}</div>
          <div style="font-size:0.6rem;color:#8A8478;margin-top:2px">{unit}</div>
        </div>
        <div style="text-align:center;background:#14181C;border-radius:6px;padding:8px 4px">
          <div style="font-size:0.6rem;color:#8A8478;margin-bottom:3px;letter-spacing:0.5px">7D MOVING AVG</div>
          <div style="font-size:1.1rem;font-weight:700;color:#C9952C;
        ...
        <div style="text-align:center;background:#14181C;border-radius:6px;padding:8px 4px">
          <div style="font-size:0.6rem;color:#8A8478;margin-bottom:3px;letter-spacing:0.5px">30D MONTHLY AVG</div>
          <div style="font-size:1.1rem;font-weight:700;color:#C9952C;
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:4px;font-size:0.65rem">
        <div style="background:#1A1E23;border-radius:4px;padding:5px 6px;text-align:center">
          <div style="color:#8A8478;margin-bottom:1px">σ (Std Dev)</div>
          <div style="color:#E8E6E1;font-weight:600">{std:.3f}</div>
        </div>
        <div style="background:#1A1E23;border-radius:4px;padding:5px 6px;text-align:center">
          <div style="color:#8A8478;margin-bottom:1px">CV%</div>
          <div style="color:#E8E6E1;font-weight:600">{cv:.1f}%</div>
        </div>
        <div style="background:#1A1E23;border-radius:4px;padding:5px 6px;text-align:center">
          <div style="color:#8A8478;margin-bottom:1px">Min – Max</div>
          <div style="color:#E8E6E1;font-weight:600">{mn:.2f}–{mx:.2f}</div>
        </div>
        <div style="background:#1A1E23;border-radius:4px;padding:5px 6px;text-align:center">
          <div style="color:#8A8478;margin-bottom:1px">Op. Limits</div>
          <div style="color:#5FB878;font-weight:600">{lo}–{hi}</div>
        </div>
      </div>
...
def trend_chart(x, y, col, label, chart_type='trend'):
    ...
    BASE_LAYOUT = dict(
        ...
        height       = 290,
        hovermode    = 'x unified',
        hoverlabel   = dict(bgcolor='#1F242A', bordercolor='#3A3530', font=dict(size=10)),
        legend       = dict(bgcolor='rgba(6,13,26,0.85)', bordercolor='#3A3530',
                            font=dict(size=9), orientation='h',
                            yanchor='bottom', y=1.01, xanchor='left', x=0),
        title=dict(
            text=f'<b>{label}</b>  <span style="font-size:10px;color:#6B6458">— {unit}</span>',
            font=dict(size=12, color='#E8E6E1'), x=0, xanchor='left'),
        xaxis = dict(gridcolor='#3A3530', showgrid=True, zeroline=False,
                     tickformat='%b %Y', dtick='M1',
                     tickfont=dict(size=9), tickangle=-30,
                     showline=True, linecolor='#3A3530',
                     title=dict(text='Date', font=dict(size=9, color='#8A8478'))),
        yaxis = dict(gridcolor='#3A3530', showgrid=True, zeroline=False,
                     title=dict(text=f'{label} ({unit})', font=dict(size=9, color='#8A8478')),
    ...
    if chart_type == 'trend':
        ...
        fig.add_hrect(y0=wlo, y1=whi,
                      ...
                      annotation_font=dict(color='#5FB878', size=8))
        ...
        fig.add_trace(go.Scatter(
            ...
            line=dict(color='#C9952C', width=1.3, dash='dot'),
        ...
        fig.add_trace(go.Scatter(
            ...
            line=dict(color='#C9952C', width=1.2),
            name='7d Moving Avg',
        ...
        for yval, name, color, dash in [
            (ucl, f'UCL {ucl:.2f}', '#D4A23B', 'dash'),
            (lcl, f'LCL {lcl:.2f}', '#D4A23B', 'dash'),
            (mu,  f'μ {mu:.2f}',    '#6B6458', 'dot'),
        ]:
        ...
        fig.add_trace(go.Scatter(
            ...
            mode='markers+text',
            marker=dict(color='#C9952C', size=10, symbol='triangle-up',
                        line=dict(color='white', width=1)),
            text=[f'▲ {s_clean[i_max]:.2f}'],
            textposition='top center',
            textfont=dict(size=8, color='#C9952C'),
        ...
        fig.update_layout(
            yaxis=dict(
                ...
                gridcolor='#3A3530', zeroline=False,
                title=dict(text=unit, font=dict(size=9, color='#8A8478')),
                tickfont=dict(size=9),
            )
        )
        return fig
    ...
    # ── SPC CONTROL CHART + HISTOGRAM ─────────────────────────────────────
        ...
        fig.add_trace(go.Histogram(
            y=yi, marker_color='#4A90D9', opacity=0.65,
        ...
        fig.update_layout(**BASE_LAYOUT)
        fig.update_layout(
            xaxis =dict(gridcolor='#3A3530', tickformat='%b %Y', dtick='M1',
                        tickfont=dict(size=9), tickangle=-30),
            yaxis =dict(gridcolor='#3A3530', tickfont=dict(size=9),
                        title=dict(text=f'{label} ({unit})', font=dict(size=9, color='#8A8478'))),
            xaxis2=dict(gridcolor='#3A3530', tickfont=dict(size=9),
                        title=dict(text='Frequency', font=dict(size=9))),
            yaxis2=dict(gridcolor='#3A3530', tickfont=dict(size=9), matches='y'),
        )
        return fig
    ...
    # ── BOX PLOT (monthly variability) ────────────────────────────────────
        ...
        fig.add_hline(y=hi, line=dict(color='#D4574A', width=1, dash='dash'))
        fig.add_hline(y=lo, line=dict(color='#D4574A', width=1, dash='dash'))
        fig.add_hline(y=mu, line=dict(color='#6B6458', width=1, dash='dot'))
        ...
        fig.update_layout(
            title=dict(text=f'<b>{label}</b> — Monthly Distribution  ({unit})',
                       font=dict(size=12, color='#E8E6E1'), x=0, xanchor='left'),
            xaxis=dict(gridcolor='#3A3530', tickfont=dict(size=9), tickangle=-30,
                       title=dict(text='Month', font=dict(size=9, color='#8A8478'))),
            yaxis=dict(gridcolor='#3A3530', tickfont=dict(size=9),
                       title=dict(text=f'{label} ({unit})', font=dict(size=9, color='#8A8478'))),
        )
        return fig
...
def render(pp_filt, pp_date, d_from, d_to):
    ...
    plant_color  = ('#5FB878' if health_score >= 80
                    else '#D4A23B' if health_score >= 60
                    else '#D4574A')
    date_label   = (f'{d_from.strftime("%d %b")} → {d_to.strftime("%d %b %Y")}')

    st.markdown(f"""
    <div style="background:linear-gradient(135deg,#1A1E23,#1F242A);
                border:1px solid #3A3530;border-radius:12px;
      ...
      <div style="display:flex;justify-content:space-between;
        ...
        <div>
          ...
          <div style="font-size:2.6rem;font-weight:800;color:{plant_color};
            ...
            {health_score:.0f}
            <span style="font-size:1rem;color:#8A8478;font-weight:400"> / 100</span>
          </div>
          <div style="font-size:0.7rem;color:#6B6458;margin-top:3px">
            {total} parameters monitored &nbsp;·&nbsp; {date_label}
          </div>
        </div>
        <div style="display:flex;gap:12px;flex-wrap:wrap">
          <div style="background:#1c0202;border:1px solid #D4574A;
                      border-radius:8px;padding:10px 22px;text-align:center">
            <div style="font-size:1.8rem;font-weight:800;color:#D4574A;
                        font-family:'JetBrains Mono',monospace">{n_crit}</div>
            <div style="font-size:0.67rem;color:#D4574A;letter-spacing:0.5px">🔴 CRITICAL</div>
          </div>
          <div style="background:#1c1200;border:1px solid #D4A23B;
                      border-radius:8px;padding:10px 22px;text-align:center">
            <div style="font-size:1.8rem;font-weight:800;color:#D4A23B;
                        font-family:'JetBrains Mono',monospace">{n_warn}</div>
            <div style="font-size:0.67rem;color:#D4A23B;letter-spacing:0.5px">🟡 WARNING</div>
          </div>
          <div style="background:#052e16;border:1px solid #5FB878;
                      border-radius:8px;padding:10px 22px;text-align:center">
            <div style="font-size:1.8rem;font-weight:800;color:#5FB878;
                        font-family:'JetBrains Mono',monospace">{n_ok}</div>
            <div style="font-size:0.67rem;color:#5FB878;letter-spacing:0.5px">🟢 NORMAL</div>
          </div>
        </div>
      </div>
    ...
    # ── LEGEND ─────────────────────────────────────────────────────────────
    st.markdown("""
    <div style="display:flex;gap:18px;flex-wrap:wrap;background:#1A1E23;
                border:1px solid #3A3530;border-radius:8px;
                padding:9px 16px;margin-bottom:14px;font-size:0.72rem;color:#B8B2A6">
      <span style="color:rgba(120,180,255,0.75)">━ Daily (raw)</span>
      <span style="color:#C9952C;font-weight:600">━━ 7d Moving Avg (primary)</span>
      <span style="color:#C9952C">┄ 30d Monthly Avg</span>
      <span style="color:#5FB878">▓ Normal Zone</span>
      <span style="color:#D4A23B">- - UCL / LCL</span>
      <span style="color:#C9952C">▲ Max</span>
      <span style="color:#06b6d4">▼ Min</span>
      <span style="color:#ffffff">● Latest</span>
      <span style="color:#D4574A">✕ Anomaly</span>
    </div>""", unsafe_allow_html=True)
    ...
    if compare_mode and len(sel) >= 2:
        ...
        if len(corr_series) >= 2:
            ...
            if pairs:
                ...
                st.markdown(
                    f'<div style="background:#1A1E23;border-left:3px solid #4A90D9;'
                    f'padding:7px 14px;font-size:0.73rem;color:#B8B2A6;'
        ...
        fig.update_layout(
            ...
            title=dict(text='Parameter Comparison — Normalised 0–100%',
                       font=dict(size=11, color='#E8E6E1'), x=0),
            xaxis=dict(gridcolor='#3A3530', tickformat='%b %Y', dtick='M1',
                       tickfont=dict(size=9), tickangle=-30),
            yaxis=dict(gridcolor='#3A3530',
                       title=dict(text='Normalised Value (%)',
                                  font=dict(size=9, color='#8A8478')),
                       tickfont=dict(size=9)),
    ...
    with bot1:
        st.markdown(
            '<div style="background:#1A1E23;border-left:3px solid #C9952C;'
        ...
        for ins in insights:
            ...
            act = (f'<div style="font-size:0.67rem;color:#8A8478;margin-top:4px">'
    ...
    with bot2:
        st.markdown(
            '<div style="background:#1A1E23;border-left:3px solid #D4574A;'
        ...
        if anom_rows:
            ...
            st.dataframe(
                adf.style
                   .map(
                       lambda v: ('color:#D4574A;font-weight:bold' if v == 'Critical'
                                  else 'color:#D4A23B' if v == 'Warning' else ''),
                       subset=['Severity'])
                   .set_properties(**{
            ...
            st.markdown(
                ...
                'color:#5FB878;font-size:0.82rem">'
